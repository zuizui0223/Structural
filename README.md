...
A separate post-outcome ecology lane uses only frozen outputs for hypothesis generation. `docs/EXPLORATORY_ECOLOGY_V0_3.md` refines the isolation-regime switch into **insular source decoupling**: topology becomes distinctive when focal-species occupied-source connectivity remains intact at scales where generic mainland stepping has already broken, including multi-hop inter-island source paths that nearest-source distance alone cannot represent. The lane remains explicitly non-confirmatory and does not change the frozen paper. The v0.2 75% extreme-isolation primary is retained unchanged; `development/prospective_extreme_isolation_topology_hypothesis_v0_3.json` adds source-conditioned excess connectivity and multi-hop source paths as non-rescuing mechanistic secondaries for a future independent panel.
