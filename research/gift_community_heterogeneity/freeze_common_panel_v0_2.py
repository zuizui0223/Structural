#!/usr/bin/env python3
"""Freeze v0.2 common-island panel with scale-free mainland step isolation.
...
def minimax_mainland_bottleneck(ids, distances, mainland_dist):
    """Minimum possible largest dispersal step from mainland to each island.

    Treat mainland as a virtual source with edge weight equal to each island's
    GIFT mainland distance. Inter-island edges use great-circle distance.
    Minimax Dijkstra returns the smallest bottleneck edge along any route.
    """
    best={eid:float(mainland_dist[eid]) for eid in ids}
    remaining=set(ids)
    while remaining:
        u=min(remaining,key=lambda eid:(best[eid],int(eid)))
        remaining.remove(u)
        for v in remaining:
            cand=max(best[u],distances[(u,v)])
            if cand < best[v]:
                best[v]=cand
    return best
...
def main():
    ...
    group_rows=[]
    step_gain_values=[]
    extreme_step_gain_values=[]
    nonextreme_step_gain_values=[]
    for path,ids in sorted(groups.items()):
        ...
        bottleneck=minimax_mainland_bottleneck(ids,pair,misc["dist"])
        islands=[]
        for eid in ids:
            ...
            direct_km=float(misc["dist"][eid])
            bottleneck_km=min(direct_km,float(bottleneck[eid]))
            topo=max(0.0,math.log1p(direct_km)-math.log1p(bottleneck_km))
            step_gain_values.append(topo)
            (extreme_step_gain_values if eid in q75 else nonextreme_step_gain_values).append(topo)
            islands.append({
                "entity_ID":eid,
                ...
                "step_bottleneck_km":bottleneck_km,
                "step_isolation_gain_log":topo,
        ...
        group_rows.append({
            ...
            "step_isolation_gain_nonzero_islands":sum(
                island["step_isolation_gain_log"]>1e-12 for island in islands
            ),
    ...
    support={
        ...
        "step_isolation_gain_nonzero_islands":sum(v>0 for v in step_gain_values),
        "extreme_step_isolation_gain_nonzero_islands":sum(v>0 for v in extreme_step_gain_values),
        "nonextreme_step_isolation_gain_nonzero_islands":sum(v>0 for v in nonextreme_step_gain_values),
        "step_isolation_gain_values":sorted(set(step_gain_values)),
    }

    payload={
        "schema":"structural.gift_community_topology_panel.v0_2",
        ...
        "reference_pressure_radii_km":list(RADII),
        "step_isolation_definition":"minimum-bottleneck path from mainland to focal island in an augmented complete graph: mainland-to-island edge weight = GIFT dist; island-to-island edge weight = great-circle distance",
        "step_isolation_gain_definition":"log1p(direct mainland distance km) - log1p(minimum bottleneck step distance km); nonnegative and zero when stepping stones do not reduce the hardest dispersal step",
        ...
        "primary_structural_predictor":"step_isolation_gain_log",
    ...
    payload["panel_fingerprint"]=sha({
        ...
        "reference_pressure_radii_km":payload["reference_pressure_radii_km"],"step_isolation_definition":payload["step_isolation_definition"],"step_isolation_gain_definition":payload["step_isolation_gain_definition"],
