...
def main():
    ...
    for base in unique:
        for clade in CLADES:
            ...
            row["step_gain_x_GMMC"]=row["step_isolation_gain_log"]*row["GMMC"]
            row["extreme_x_GMMC"]=row["extreme"]*row["GMMC"]
            row["step_gain_x_extreme_x_GMMC"]=row["step_gain_x_extreme"]*row["GMMC"]
    ...
    h2_cols=h1_cols+[
        "step_gain_x_GMMC","extreme_x_GMMC","step_gain_x_extreme_x_GMMC"
    ]
    h3_cols=h1_cols+[
    ...
    protocol={
        "schema":"structural.gift_community_topology_protocol.v0_2",
        "status":"QUALIFIED_TO_OPEN_COMMUNITY_RESPONSE" if qualified else "STOP_PRE_RESPONSE_NON_ESTIMABLE",
        "study_family":"fresh community-level island-biogeography study; v0.2 replaces predictor-only non-estimable v0.1 without any response access",
        "gift_version":"3.2",
        "clades":list(CLADES),
        ...
        "H2_primary":{
            ...
            "estimand":"coefficient of standardized step-isolation gain x extreme x GMMC in a hierarchical interaction model that also contains step_isolation_gain_log x GMMC and extreme x GMMC",
            "prediction":"negative: the extreme-isolation step-isolation benefit is weaker on islands connected to mainland at the Last Glacial Maximum",
        ...
        "pre_response_predecessor":{
            ...
            "response_values_accessed_in_v0_1":False,
        },
