#!/usr/bin/env python3
"""Freeze and preflight v0.2 scale-free step-isolation richness protocol."""
...
def main():
    ...
    ap.add_argument("--geology-crosswalk",type=Path,required=True)
    a=ap.parse_args()
    panel=load(a.panel); prior=load(a.prior_exclusions); geology=load(a.geology_crosswalk)
    ...
    if geology.get("status")!="FROZEN_BEFORE_COMMUNITY_RESPONSE":
        raise RuntimeError("geology crosswalk not frozen")
    if geology.get("panel_fingerprint")!=panel["panel_fingerprint"]:
        raise RuntimeError("geology crosswalk panel drift")
    if geology.get("response_values_accessed") is not False:
        raise RuntimeError("geology crosswalk opened response")
    geology_by={g["archipelago_id"]:g for g in geology["groups"]}
        ...
        for island in g["islands"]:
            ...
            unique.append({
                ...
                **p,
                ...
                "oceanic_fraction":geology_by.get(g["archipelago_id"],{}).get("oceanic_fraction"),
            })
    ...
    for base in unique:
        for clade in CLADES:
            ...
            if row["oceanic_fraction"] is not None:
                row["step_gain_x_oceanic"]=row["step_isolation_gain_log"]*row["oceanic_fraction"]
                row["extreme_x_oceanic"]=row["extreme"]*row["oceanic_fraction"]
                row["step_gain_x_extreme_x_oceanic"]=row["step_gain_x_extreme"]*row["oceanic_fraction"]
            else:
                row["step_gain_x_oceanic"]=None
                row["extreme_x_oceanic"]=None
                row["step_gain_x_extreme_x_oceanic"]=None
    ...
    h1_cols=list(REFERENCE_COLS)+["step_isolation_gain_log","extreme","step_gain_x_extreme"]
    h2_cols=h1_cols+[
        "step_gain_x_oceanic","extreme_x_oceanic","step_gain_x_extreme_x_oceanic"
    ]
    h3_cols=h1_cols+[
        "step_gain_x_fern","extreme_x_fern","step_gain_x_extreme_x_fern"
    ]
    h2_rows=[row for row in island_rows if row["oceanic_fraction"] is not None]
    audits={
        "H1":matrix_audit(island_rows,h1_cols),
        "H2":matrix_audit(h2_rows,h2_cols),
    ...
    gates={
        ...
        "topology_support_extreme":support["extreme_step_isolation_gain_nonzero_islands"]>=20,
        "topology_support_nonextreme":support["nonextreme_step_isolation_gain_nonzero_islands"]>=20,
        ...
        "H2_geology_support":geology.get("continuous_H2_support") is True,
        "H2_full_rank":audits["H2"]["full_rank"] and audits["H2"]["condition"]<=MAX_CONDITION,
        "H2_target_retained":"step_gain_x_extreme_x_oceanic" in audits["H2"]["columns"],
    ...
    protocol={
        "schema":"structural.gift_community_topology_protocol.v0_4",
        "status":"QUALIFIED_TO_OPEN_COMMUNITY_RESPONSE" if qualified else "STOP_PRE_RESPONSE_NON_ESTIMABLE",
        "study_family":"fresh community-level island-biogeography study; v0.4 preserves H1/H3 and uses the published Roeble cleaned-archipelago geological origin as the final pre-response H2 moderator"
        "gift_version":"3.2",
        "clades":list(CLADES),
        ...
        "H1_primary":{
            ...
            "estimand":"coefficient of standardized step-isolation gain x global-extreme-isolation after the declared reference and archipelago x clade fixed effects",
            "prediction":"positive: a smaller minimum stepping-stone bottleneck buffers richness loss more strongly under extreme mainland isolation",
            "success_rule":"95% archipelago-bootstrap interval for step_gain_x_extreme excludes 0 on the positive side",
        },
        "H2_primary":{
            "model_columns":h2_cols,
            "analysis_population":"archipelagos with >=5 exact GIFT entity_ID matches, >=50% entity match coverage, and >=80% of matched entities assigned to one Roeble cleaned archipelago",
            "moderator":"archipelago oceanic_fraction from exact entity_ID geological type crosswalk",
            "geology_crosswalk_fingerprint":geology["crosswalk_fingerprint"],
            "geology_support":{
                "eligible_archipelagos":geology["eligible_archipelagos"],
                "oceanic_fraction_min":geology["oceanic_fraction_min"],
                "oceanic_fraction_max":geology["oceanic_fraction_max"],
                "pure_continental":geology["pure_continental"],
                "pure_oceanic":geology["pure_oceanic"],
                "mixed":geology["mixed"],
            },
            "estimand":"coefficient of standardized step-isolation gain x extreme x oceanic_fraction in a hierarchical interaction model that also contains step-isolation gain x oceanic_fraction and extreme x oceanic_fraction",
            "prediction":"positive: the extreme-isolation step-isolation benefit strengthens as an archipelago is more oceanic and less historically connected to continental land",
            "success_rule":"95% archipelago-bootstrap interval excludes 0 on the positive side",
            "inference":"whole-archipelago bootstrap restricted to geology-eligible archipelagos",
            "claim_ceiling":"geological assembly-history moderator, not direct colonization/extinction mechanism",
        },
        "H3_primary":{
            "model_columns":h3_cols,
            "fern_indicator":"Pteridophyta=1; Angiospermae/Gymnospermae=0",
            "estimand":"coefficient of standardized step-isolation gain x extreme x fern in a hierarchical interaction model that also contains step_isolation_gain_log x fern and extreme x fern",
            "prediction":"negative: high-dispersal ferns show weaker dependence on stepping-stone reduction of effective isolation than seed plants",
        ...
        "pre_response_predecessor":{
            "v0_1_receipt":"research/gift_community_topology/pre_response_stop_v0_1.json",
            "v0_1_failed_because":"fixed-radius topology gain had zero support in all 97 global-extreme islands",
            "v0_2_receipt":"research/gift_community_topology/pre_response_h2_stop_v0_2.json",
            "v0_2_H1_H3_estimable":True,
            "v0_2_H2_failed_because":"GMMC was constant in the relevant global-extreme interaction, so the declared H2 target column was dropped before response",
            "v0_3_receipt":"research/gift_community_topology/pre_response_h2_stop_v0_3.json",
            "v0_3_H2_failed_because":"direct island-type coverage >=80% left the geological interaction aliased under archipelago x clade fixed effects",
            "v0_4_crosswalk":"Roeble cleaned archipelago grouping via exact entity_ID matches; no fuzzy name matching",
            "response_values_accessed_in_v0_1_v0_2_or_v0_3":False,
        },
        "literature_basis":"scale-free step isolation is the minimum dispersal step needed to reach an island via mainland/island stepping stones; v0.2 uses the exact minimax generalization on the full archipelago graph",
        ...
        "forbidden_after_response":[
            ...
            "change Roeble exact entity_ID / >=5 matches / >=50% coverage / >=80% dominant cleaned-archipelago matching rule",
            "change global q75 isolation threshold","change 25/50/125/250 km graph radii",
            "change minimum-bottleneck step-isolation gain definition","drop reference covariates after seeing richness",
