...
def main()->int:
    ...
    frozen_bootstrap=lock.get("bootstrap",{})
    if frozen_bootstrap.get("accepted_draws_sha256")!=protocol["model"]["bootstrap_design"]["accepted_draws_sha256"]:
        raise RuntimeError("bootstrap draw-set drift")
    if int(frozen_bootstrap.get("candidate_draws_attempted",-1))!=int(protocol["model"]["bootstrap_design"]["candidate_draws_attempted"]):
