...
QUEUE_SCHEMA = "structural.confirmatory_admission_queue.v0_36"
ENTRY_KEYS = {
    "system_id",
    "protocol_id",
    "protocol_path",
    ...
    for index, entry in enumerate(entries):
        ...
        if decision.status is not ConfirmatoryAdmissionStatus.ADMITTED:
            raise QueueValidationError(
                f"entry {index} fails v0.31-v0.33: {', '.join(decision.reasons)}"
            )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="?",
        default=ROOT / "development/confirmatory_admission_queue_v0_36.json",
    )
    args = parser.parse_args()
    try:
        count = validate_queue(args.queue)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        print(f"confirmatory queue validation failed: {exc}", file=sys.stderr)
        return 1
    print(f"confirmatory queue valid: {count} admitted system(s)")
    return 0
